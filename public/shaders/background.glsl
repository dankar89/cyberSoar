...
const float fogOffset = 6.0;
const float fogDensity = 0.9;
// const vec3 fogColor = vec3(0.25, 0.0, 0.3);
const vec3 fogColor = vec3(0.21, 0.19, 0.0);
...
vec3 addSign(vec3 color, vec3 pos, float side, vec2 id) {
  ...
  flash = smoothstep(0.1, 0.5, 0.5 + 0.5 * cos(flash * u_time));
  ...
  flicker = 1.0 - flicker * step(0.96, hash1(charId, u_time));
